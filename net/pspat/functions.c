...
pspat_do_arbiter(struct pspat *arb)
{
	...
	for (q = arb->qdiscs; q; q = q->pspat_next) {
		...
		while (next_link_idle <= now && ndeq < pspat_arb_qdisc_batch) {
		...
		if (next_link_idle <= now && ndeq < pspat_arb_qdisc_batch) {
			next_link_idle = now;
		}
...
pspat_do_dispatcher(struct pspat_dispatcher *s)
{
	...
	while (ndeq < pspat_dispatch_batch
	       && (skb = pspat_mb_extract(m)) != NULL) {
